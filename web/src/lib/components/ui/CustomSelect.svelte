...
>
	...
	>
		...
		<span class="ml-2 select-arrow">
			<svg
				class="h-5 w-5 text-green-500 transition-transform duration-300 {isOpen
					? 'rotate-180'
					: ''}"
				fill="currentColor"
				viewBox="0 0 20 20"
	...
	{#if isOpen}
		...
		>
			{#each options as option}
				<div
					class="py-2.5 px-4 cursor-pointer hover:bg-green-900/30 transition-colors flex items-center relative group {option.value ===
					value
				...
				>
					<div
						class="absolute left-0 top-0 bottom-0 w-0.5 bg-green-500/0 group-hover:bg-green-500/70 transition-all {option.value ===
						value
							? 'bg-green-500/70'
							: ''}"
					></div>
