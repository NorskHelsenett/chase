...
<div use:lazyCheck={imageUrl} class="w-full h-full">
  ...
  {:else if imageErrors[imageUrl]}
    <div class="absolute inset-0 flex flex-col items-center justify-center bg-black/70 text-gray-300 rounded-xl p-4 backdrop-blur-sm">
      <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mb-3 text-gray-400 opacity-60">
        <rect width="18" height="18" x="3" y="3" rx="2" ry="2"></rect>
        <circle cx="9" cy="9" r="2"></circle>
        <path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"></path>
      </svg>
      <span class="text-center font-medium">Failed to load screenshot</span>
      <span class="text-xs text-gray-400 mt-1">{new URL(site.url.startsWith('http') ? site.url : `https://${site.url}`).hostname}</span>
    </div>
  {:else}
    <div class="absolute inset-0 bg-gradient-to-br from-gray-900/80 to-black/90 rounded-lg flex flex-col items-center justify-center">
      <div class="relative">
