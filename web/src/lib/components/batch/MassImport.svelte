...
<script lang="ts">
	...
	function showFailedImports() {
		if (importStats && importStats.failed > 0 && importModalComponent) {
			console.log('Showing failed imports modal');
			importModalComponent.showFailedImports(importStats.failedDetails || []);
		}
	}
</script>
...
<div class="space-y-6">
	<div class="bg-[#202020] rounded-lg p-4">
		...
		<p class="mt-2 text-gray-400 text-sm">
			Add multiple servers at once with shared monitoring settings, or update existing servers in
			bulk.
		</p>

		{#if showStats}
			<div class="mt-4 p-3 bg-green-900/30 border border-green-800/50 rounded-lg">
