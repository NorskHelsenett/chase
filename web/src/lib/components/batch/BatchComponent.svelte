	...
	function startPolling(jobId: string) {
		pollingIntervals[jobId] = window.setInterval(async () => {
			try {
				...
				activeJobs = activeJobs.map((job) => (job.id === jobId ? { ...job, ...jobStatus } : job));
					...
					<div class="space-y-2">
						<div class="h-2 bg-[#2b2b2b] rounded-full overflow-hidden">
							<div
								...
								style="width: {((job.completed + (job.errors ? job.errors?.length : 0)) /
									job.total) *
									100}%"
							>
								<div
			...
			<div class="p-4">
				<div class="p-4 rounded-lg space-y-2 max-h-[60vh] overflow-y-auto">
					{#each selectedError as error}
						<pre
							class="text-sm text-red-400 whitespace-pre-wrap p-2 rounded bg-[#2b2b2b]">{error}</pre>
					{/each}
				</div>
			</div>
